...
import { generateItineraryStreamAction } from "@/features/generateLLM/generateAction";
import { withApiSecurity, type SecurityValidationResult } from "@/lib/security";

/**
 * Secure streaming itinerary generation endpoint
...
async function handleStreamingGeneration(
  ...
  console.log(`🚀 Processing streaming generation for user: ${validationResult.userId}`);
  ...
  try {
    ...
    // Call the streaming generation action with request for cancellation support
    const result = await generateItineraryStreamAction(formData, request);
    ...
    console.error("Error in secure streaming generation:", error);
...
// Apply security wrapper - temporarily disable CSRF for debugging
export const POST = withApiSecurity(handleStreamingGeneration, {
  requireAuth: true,
