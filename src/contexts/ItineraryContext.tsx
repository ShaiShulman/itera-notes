...
const extractTitleFromEditorData = (
  ...
  try {
    ...
    const headerBlock = editorData.blocks.find(
      (block) => block && block.type === "header"
    );
...
function itineraryReducer(
  ...
  switch (action.type) {
    ...
    case "LOAD_ITINERARY":
      const newContentHash = action.payload.editorData
        ? generateContentHash(action.payload.editorData)
        : null;
      return {
        ...state,
        ...
        currentItinerary: action.payload.editorData
          ? convertEditorDataToItinerary(action.payload.editorData)
          : null,
    ...
    case "SET_EDITOR_DATA":
      const editorContentHash = action.payload
        ? generateContentHash(action.payload)
        : null;
      ...
      return {
        ...
        currentItinerary: action.payload
          ? convertEditorDataToItinerary(action.payload)
          : null,
    ...
    case "UPDATE_EDITOR_DATA":
      const updateContentHash = action.payload
        ? generateContentHash(action.payload)
        : null;
      ...
      autoSaveTimeoutId = setTimeout(async () => {
        try {
          ...
          if (response.success) {
            ...
            dispatch({
              type: "SET_ERROR",
              payload: "Failed to save itinerary",
            });
          }
        } catch (error) {
  ...
  const loadItineraryById = useCallback(async (id: string) => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        dispatch({
          type: "SET_ERROR",
          payload: response.error || "Failed to load itinerary",
        });
      }
    } catch (error) {
